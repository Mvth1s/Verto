...
const categoryName = computed(() =>
  activeCategory.value === 'images' ? 'Images' : 'Documents',
)
    ...
    <div class="shell">
      ...
      <aside class="sidebar">
        ...
        <nav class="nav">
          ...
          <div class="nav-item disabled">
            <svg viewBox="0 0 24 24"><rect x="2" y="6" width="14" height="12" rx="2"/><path d="M22 8l-6 4 6 4z"/></svg>
            <span>Video</span>
            <span class="soon">Soon</span>
          </div>
        </nav>
      ...
      <main class="main">
        ...
        >
          ...
          <div>
            ...
            <div class="drop-sub" style="text-align:center;margin-top:4px;">or click to browse · max 100 files at once</div>
          </div>
          <div class="drop-kbd">
            <kbd>⌘</kbd><kbd>O</kbd>
          </div>
        ...
        <div v-if="conversion.queue.length > 0" class="queue">
          <div class="queue-header">
            ...
            <div class="queue-actions">
              <button class="queue-clear" @click="conversion.clearDone" v-if="conversion.done.length > 0">
                Clear done
              </button>
          ...
          >
            ...
            <div class="fname">
              ...
              <span class="to">{{ file.name.replace(/\.[^/.]+$/, '') }}.{{ settings.outputFormat }}</span>
            </div>
            <div
              class="fstatus"
              :class="{
            ...
            >
              <svg v-if="file.status === 'done'" viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>
              <svg v-else-if="file.status === 'converting'" viewBox="0 0 24 24"><circle cx="12" cy="12" r="9"/><path d="M12 8v4l3 2"/></svg>
              <svg v-else-if="file.status === 'error'" viewBox="0 0 24 24"><circle cx="12" cy="12" r="9"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
      ...
      <aside class="panel">
        ...
        <div class="field">
          ...
          <select class="select" v-model="settings.outputFormat">
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .nav-item.disabled { color: var(--text-3); cursor: not-allowed; opacity: 0.55; pointer-events: none; }
  .nav-item .soon {
    margin-left: auto;
    font-size: 9px;
    text-transform: uppercase;
  ...
  .nav-item svg { width: 15px; height: 15px; stroke: currentColor; stroke-width: 1.6; fill: none; flex-shrink: 0; }
  ...
  .pill-version .dot { width: 6px; height: 6px; border-radius: 50%; background: var(--accent); box-shadow: 0 0 8px var(--accent); }
  .icon-btn {
  ...
  .main-title { font-size: 19px; font-weight: 600; letter-spacing: -0.015em; }
  .main-sub { font-size: 12px; color: var(--text-3); font-family: 'JetBrains Mono', monospace; }
  ...
  .drop-icon svg { width: 26px; height: 26px; stroke: currentColor; stroke-width: 1.5; fill: none; }
  .drop-title { font-size: 15px; font-weight: 500; color: var(--text); letter-spacing: -0.005em; }
  ...
  .queue-title { font-size: 11px; text-transform: uppercase; letter-spacing: 0.1em; color: var(--text-3); font-weight: 500; }
  .queue-meta { font-family: 'JetBrains Mono', monospace; font-size: 11px; color: var(--text-3); }
  .queue-actions { display: flex; align-items: center; gap: 12px; }
  .queue-clear { background: transparent; border: none; color: var(--text-3); font: inherit; font-size: 11px; cursor: pointer; padding: 2px 6px; border-radius: 4px; }
  .queue-clear:hover { color: var(--text-2); background: rgba(255,255,255,0.04); }
  .queue-row {
    display: grid;
  ...
  .queue-row .fname span:first-child { overflow: hidden; text-overflow: ellipsis; }
  .queue-row .fname .arrow { color: var(--text-3); flex-shrink: 0; }
  .queue-row .fname .to { color: var(--accent-bright); flex-shrink: 0; }
  .queue-row .fstatus { font-size: 11px; font-family: 'JetBrains Mono', monospace; color: var(--text-3); text-align: right; }
  .queue-row .fstatus.done { color: var(--accent-bright); }
  .queue-row .fstatus.progress { color: var(--text-2); }
  .queue-row .fstatus.error { color: var(--danger); }
  .qaction { display: grid; place-items: center; width: 22px; height: 22px; border-radius: 4px; color: var(--text-3); cursor: pointer; }
  .qaction:hover { background: rgba(255,255,255,0.06); color: var(--text-2); }
  .qaction.check { color: var(--accent-bright); }
  .qaction svg { width: 14px; height: 14px; stroke: currentColor; stroke-width: 1.8; fill: none; }
  ...
  .field-label { display: flex; justify-content: space-between; font-size: 12px; color: var(--text-2); margin-bottom: 8px; font-weight: 500; }
  .field-label .val { font-family: 'JetBrains Mono', monospace; color: var(--text); }
  .field-hint { font-size: 10px; color: var(--text-3); margin-top: 5px; font-style: italic; }
  ...
  .slider-track-wrap { position: relative; padding: 6px 0; }
  .ticks { display: flex; justify-content: space-between; font-family: 'JetBrains Mono', monospace; font-size: 10px; color: var(--text-3); margin-top: 6px; }
  ...
  .folder-path { flex: 1; padding: 9px 12px; font-family: 'JetBrains Mono', monospace; font-size: 11px; color: var(--text-2); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .folder-browse { border: none; border-left: 1px solid var(--border); background: transparent; color: var(--text-2); font: inherit; font-size: 12px; padding: 9px 12px; cursor: pointer; font-weight: 500; }
  .folder-browse:hover { color: var(--text); background: rgba(255,255,255,0.03); }
  ...
  .summary-key { font-size: 10px; text-transform: uppercase; letter-spacing: 0.1em; color: var(--text-3); font-weight: 500; }
  .summary-val { font-family: 'JetBrains Mono', monospace; font-size: 12px; color: var(--text); }
  ...
  .btn-primary svg { width: 14px; height: 14px; stroke: currentColor; stroke-width: 2.2; fill: none; }
